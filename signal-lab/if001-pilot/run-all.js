...
// IF-001-PILOT — Signal Lab Collection Orchestrator
//
// Reads the 100-firm cohort manifest (cohort-manifest.json), runs all 9
// Signal Lab collectors against each domain, inserts observations into the
// approved Signal Lab tables, and writes IF001_PILOT_COLLECTION_REPORT.md.
//
...
//
// Usage:
//   node backend/signal-lab/if001-pilot/run-all.js
//
// Environment:
...
const COHORT_CODE        = 'IF-001-PILOT';
const COLLECTOR_VERSION  = '1.0.0';
const MANIFEST_PATH      = path.join(__dirname, 'cohort-manifest.json');
const REPORT_PATH        = path.join(__dirname, '../../../IF001_PILOT_COLLECTION_REPORT.md');
...
// ── Signal definitions ────────────────────────────────────────────────────────
//
// Each signal definition maps the collector's return object to a Supabase insert.
// v0 signals (SPF→CAA): no run_id in schema; tracked by collected_at range.
// v1 signals (SecurityTXT, SecurityHeaders): run_id stored in schema.
...
async function runSpf(firms, supabase, runId, startedAt) {
  const results = await runWithConcurrency(firms, async (firm, i) => {
    try {
      ...
      const sym = f.spf_present === true ? '✓' : f.spf_present === false ? '∅' : '?';
      console.log(`  [${pct}%] ${sym}  ${firm.domain.padEnd(40)}${error ? ` [DB:${error.message.slice(0, 30)}]` : ''}`);
      return { domain: firm.domain, success: !error, collectorError: null, dbError: error?.message ?? null };
    } catch (err) {
      const pct = String(Math.round(((i + 1) / firms.length) * 100)).padStart(3);
      console.log(`  [${pct}%] !  ${firm.domain.padEnd(40)} [COLLECTOR:${err.message.slice(0, 40)}]`);
...
async function runDkim(firms, supabase, runId, startedAt) {
  ...
  const results = await runWithConcurrency(firms, async (firm, i) => {
    try {
      ...
      const pct = String(Math.round(((i + 1) / firms.length) * 100)).padStart(3);
      const sym = f.dkim_present === true ? '✓' : f.dkim_collection_status === 'NOT_DETECTED' ? '∅' : '?';
      console.log(`  [${pct}%] ${sym}  ${firm.domain.padEnd(40)}${error ? ` [DB:${error.message.slice(0, 30)}]` : ''}`);
      return { domain: firm.domain, success: !error, collectorError: null, dbError: error?.message ?? null };
    } catch (err) {
      const pct = String(Math.round(((i + 1) / firms.length) * 100)).padStart(3);
...
async function runDmarc(firms, supabase, runId, startedAt) {
  ...
  const results = await runWithConcurrency(firms, async (firm, i) => {
    try {
      ...
      const pct = String(Math.round(((i + 1) / firms.length) * 100)).padStart(3);
      const sym = f.dmarc_present === true ? 'Y' : f.dmarc_present === false ? 'N' : '?';
      console.log(`  [${pct}%] ${sym}  ${firm.domain.padEnd(40)}${error ? ` [DB:${error.message.slice(0, 30)}]` : ''}`);
      return { domain: firm.domain, success: !error, collectorError: null, dbError: error?.message ?? null };
    } catch (err) {
      const pct = String(Math.round(((i + 1) / firms.length) * 100)).padStart(3);
...
async function runMtaSts(firms, supabase, runId, startedAt) {
  ...
  const results = await runWithConcurrency(firms, async (firm, i) => {
    try {
      ...
      const pct = String(Math.round(((i + 1) / firms.length) * 100)).padStart(3);
      const sym = f.dns_sts_present === true ? 'Y' : f.dns_sts_present === false ? 'N' : '?';
      console.log(`  [${pct}%] ${sym}  ${firm.domain.padEnd(40)}${error ? ` [DB:${error.message.slice(0, 30)}]` : ''}`);
      return { domain: firm.domain, success: !error, collectorError: null, dbError: error?.message ?? null };
    } catch (err) {
      const pct = String(Math.round(((i + 1) / firms.length) * 100)).padStart(3);
...
async function runTlsRpt(firms, supabase, runId, startedAt) {
  ...
  const results = await runWithConcurrency(firms, async (firm, i) => {
    try {
      ...
      const pct = String(Math.round(((i + 1) / firms.length) * 100)).padStart(3);
      const sym = r.dns_tlsrpt_present === true ? 'Y' : r.dns_tlsrpt_present === false ? 'N' : '?';
      console.log(`  [${pct}%] ${sym}  ${firm.domain.padEnd(40)}${error ? ` [DB:${error.message.slice(0, 30)}]` : ''}`);
      return { domain: firm.domain, success: !error, collectorError: null, dbError: error?.message ?? null };
    } catch (err) {
      const pct = String(Math.round(((i + 1) / firms.length) * 100)).padStart(3);
...
async function runDnssec(firms, supabase, runId, startedAt) {
  const results = await runWithConcurrency(firms, async (firm, i) => {
    try {
      ...
      const pct = String(Math.round(((i + 1) / firms.length) * 100)).padStart(3);
      const sym = r.dns_ds_present === true ? 'DS:Y' : r.dns_ds_present === false ? 'DS:N' : 'DS:?';
      console.log(`  [${pct}%]  ${firm.domain.padEnd(40)} ${sym}${error ? ` [DB:${error.message.slice(0, 30)}]` : ''}`);
      return { domain: firm.domain, success: !error, collectorError: null, dbError: error?.message ?? null };
    } catch (err) {
      const pct = String(Math.round(((i + 1) / firms.length) * 100)).padStart(3);
      console.log(`  [${pct}%]  ${firm.domain.padEnd(40)} [COLLECTOR:${err.message.slice(0, 40)}]`);
...
async function runCaa(firms, supabase, runId, startedAt) {
  const results = await runWithConcurrency(firms, async (firm, i) => {
    try {
      ...
      const pct = String(Math.round(((i + 1) / firms.length) * 100)).padStart(3);
      const sym = r.dns_caa_present === true ? 'Y' : r.dns_caa_present === false ? 'N' : '?';
      console.log(`  [${pct}%] ${sym}  ${firm.domain.padEnd(40)}${error ? ` [DB:${error.message.slice(0, 30)}]` : ''}`);
      return { domain: firm.domain, success: !error, collectorError: null, dbError: error?.message ?? null };
    } catch (err) {
      const pct = String(Math.round(((i + 1) / firms.length) * 100)).padStart(3);
...
async function runSecurityTxt(firms, supabase, runId, startedAt) {
  const results = await runWithConcurrency(firms, async (firm, i) => {
    try {
      ...
      const pct = String(Math.round(((i + 1) / firms.length) * 100)).padStart(3);
      const sym = record.file_state === 'ABSENT' ? 'N' :
                  record.file_state.startsWith('PRESENT') ? 'Y' : '?';
      console.log(`  [${pct}%] ${sym}  ${firm.domain.padEnd(40)} ${record.file_state}${error ? ` [DB:${error.message.slice(0, 30)}]` : ''}`);
      return { domain: firm.domain, success: !error, collectorError: null, dbError: error?.message ?? null };
    } catch (err) {
      const pct = String(Math.round(((i + 1) / firms.length) * 100)).padStart(3);
      console.log(`  [${pct}%] !  ${firm.domain.padEnd(40)} [COLLECTOR:${err.message.slice(0, 40)}]`);
...
async function runSecurityHeaders(firms, supabase, runId, startedAt) {
  const results = await runWithConcurrency(firms, async (firm, i) => {
    try {
      ...
      const esTag = { RESPONSE_OBSERVED: 'RESP', TLS_ERROR: 'TLS!', CONNECTION_ERROR: 'CONN!', TIMEOUT: 'TIME!', REDIRECT_UNRESOLVED: 'RDR!' }[es] ?? es.slice(0, 5);
      const present = Object.values(record.header_inventory).filter(o => o.present).length;
      console.log(`  [${pct}%]  ${firm.domain.padEnd(40)} [${esTag}] ${String(present).padStart(2)}/25${error ? ` [DB:${error.message.slice(0, 25)}]` : ''}`);
      return { domain: firm.domain, success: !error, collectorError: null, dbError: error?.message ?? null };
    } catch (err) {
      const pct = String(Math.round(((i + 1) / firms.length) * 100)).padStart(3);
      console.log(`  [${pct}%] !  ${firm.domain.padEnd(40)} [COLLECTOR:${err.message.slice(0, 40)}]`);
...
function generateReport(manifest, allStats, globalStart, globalEnd) {
  ...
  const passingSignals = allStats.filter(s => s.successRate >= 95).length;
  const decisionGate   = passingSignals === 9 &&
    allStats.every(s => s.collectorErrors === 0 || s.successRate >= 95);

  const lines = [];

  lines.push(`# IF001_PILOT_COLLECTION_REPORT`);
  lines.push('');
  ...
  lines.push(`**Domains in pilot:** ${manifest.n}`);
  lines.push(`**Pool size (FCA prospects):** ${manifest.pool_size}`);
  lines.push(`**Execution model:** PARALLEL`);
  lines.push(`**Signal concurrency limit:** ${SIGNAL_CONCURRENCY}`);
  lines.push('');
  lines.push('---');
  lines.push('');
  lines.push('## 1. Signal Collection Summary');
  lines.push('');
  lines.push('| Signal | Run ID | Attempted | Completed | Collector Errors | DB Errors | Success Rate | Runtime |');
  lines.push('|---|---|---|---|---|---|---|---|');

  for (const s of allStats) {
    const runIdShort = s.runId ? s.runId.slice(0, 8) + '…' : '_not stored_';
    const schemaGen  = ['SOT-SECURITYTXT-001', 'SOT-SECURITYHEADERS-001'].includes(s.signalId) ? 'v1' : 'v0';
    const runIdNote  = schemaGen === 'v0' ? `${runIdShort} _(not in DB — v0 schema)_` : s.runId;
    lines.push(`| ${s.signalId} | \`${runIdNote}\` | ${s.attempted} | ${s.completed} | ${s.collectorErrors} | ${s.dbErrors} | **${s.successRate}%** | ${s.elapsedSec}s |`);
  }

  lines.push('');
  lines.push(`**Overall: ${overallCompleted}/${overallAttempted} observations stored (${overallRate}%)**`);
  lines.push('');
  lines.push('---');
  lines.push('');
  lines.push('## 2. Domains Attempted');
  lines.push('');
  lines.push(`${manifest.n} domains were selected from the IF-001-PILOT cohort manifest.`);
  lines.push('');
  lines.push('| # | Domain | Firm Name |');
  lines.push('|---|---|---|');
  manifest.firms.forEach((f, i) => {
    lines.push(`| ${i + 1} | ${f.domain} | ${f.firm_name} |`);
  });
  lines.push('');
  lines.push('---');
  lines.push('');
  lines.push('## 3. Signal Completion Rates');
  lines.push('');

  for (const s of allStats) {
    const bar = '█'.repeat(Math.round(s.successRate / 5));
    lines.push(`**${s.signalId}**`);
    lines.push(`- Completed: ${s.completed}/${s.attempted} (${s.successRate}%)`);
    lines.push(`- Collector errors: ${s.collectorErrors}`);
    lines.push(`- Database write errors: ${s.dbErrors}`);
    lines.push(`- Runtime: ${s.elapsedSec}s`);
    lines.push(`- \`${bar}\` ${s.successRate}%`);
    lines.push('');
  }

  lines.push('---');
  lines.push('');
  lines.push('## 4. Collection Failures');
  lines.push('');
  ...
  if (allFailures.length === 0) {
    ...
    lines.push(`${allFailures.length} failure(s) recorded across all signals:\n`);
    lines.push('| Signal | Domain | Error type | Detail |');
    lines.push('|---|---|---|---|');
    for (const f of allFailures) {
      const type   = f.collectorError ? 'COLLECTOR_ERROR' : 'DB_WRITE_ERROR';
      const detail = (f.collectorError ?? f.dbError ?? '').slice(0, 80);
  ...
  lines.push('---');
  lines.push('');
  lines.push('## 5. Runtime Statistics');
  lines.push('');
  lines.push(`| Signal | Runtime (s) | Domains/s |`);
  lines.push('|---|---|---|');
  for (const s of allStats) {
    const throughput = s.elapsedSec > 0 ? (s.attempted / s.elapsedSec).toFixed(1) : '—';
    lines.push(`| ${s.signalId} | ${s.elapsedSec} | ${throughput} |`);
  }
  lines.push(`| **TOTAL** | **${totalElapsed}** | — |`);
  lines.push('');
  ...
  lines.push('### 5.1 Parallelism Validation');
  lines.push('');
  ...
  lines.push(`| Sequential baseline (COP-002 §5) | 203.4s total; ratio 1.000 |`);
  lines.push('');
  lines.push('> ratio ≈ 1.000 confirms sequential execution; ratio < 1.000 confirms parallel speedup.');
  lines.push('> Compare against COP-002 §5 baseline per ADR-COL-002 Constraint 3. Record in RUN_REGISTER.md.');
  lines.push('');
  lines.push('---');
  lines.push('');
  lines.push('## 6. Schema Validation Status');
  lines.push('');
  lines.push('| Signal | Schema | Table | Run ID in DB | Status |');
  lines.push('|---|---|---|---|---|');
  for (const s of allStats) {
    const isV1 = ['SOT-SECURITYTXT-001', 'SOT-SECURITYHEADERS-001'].includes(s.signalId);
    const table = {
    ...
    const runInDb = isV1 ? 'Yes — run_id stored' : 'No — v0 schema (track by collected_at)';
    const status  = s.dbErrors === 0 && s.collectorErrors === 0 ? 'PASS' :
                    s.successRate >= 95 ? 'PASS (minor failures)' : 'REVIEW';
    lines.push(`| ${s.signalId} | ${isV1 ? 'v1' : 'v0'} | \`${table}\` | ${runInDb} | **${status}** |`);
  }
  lines.push('');
  lines.push('---');
  lines.push('');
  lines.push('## 7. Collector Issues Encountered');
  lines.push('');

  const issuesBySignal = allStats.filter(s => s.collectorErrors > 0 || s.dbErrors > 0);
  if (issuesBySignal.length === 0) {
    lines.push('No collector or database issues encountered.');
  } else {
    for (const s of issuesBySignal) {
      lines.push(`**${s.signalId}**`);
      if (s.collectorErrors > 0) {
        lines.push(`- ${s.collectorErrors} collector exception(s)`);
        for (const f of s.failures.filter(f => f.collectorError)) {
          lines.push(`  - \`${f.domain}\`: ${f.collectorError.slice(0, 100)}`);
        }
      }
      if (s.dbErrors > 0) {
        lines.push(`- ${s.dbErrors} database write error(s)`);
        for (const f of s.failures.filter(f => f.dbError)) {
          lines.push(`  - \`${f.domain}\`: ${f.dbError.slice(0, 100)}`);
        }
      }
      lines.push('');
    }
  ...
  lines.push('---');
  lines.push('');
  lines.push('## 8. Decision Gate Assessment');
  lines.push('');
  lines.push('**Gate criterion:** ≥95% success rate on all 9 signals, no methodology or schema issues.');
  lines.push('');
  lines.push('| Signal | Success Rate | Gate |');
  lines.push('|---|---|---|');
  for (const s of allStats) {
    const pass = s.successRate >= 95;
    lines.push(`| ${s.signalId} | ${s.successRate}% | ${pass ? '✓ PASS' : '✗ FAIL'} |`);
  }
  lines.push('');

  if (decisionGate) {
    lines.push('### RECOMMENDATION: PROCEED TO FULL COHORT');
    lines.push('');
    lines.push('All 9 signals met the ≥95% success threshold. No methodology or schema issues observed.');
    lines.push('IF-001 full cohort (1,928 validated FCA domains) is eligible for Signal Lab collection.');
    lines.push('');
    lines.push('**Awaiting approval before executing full cohort collection.**');
  } else {
    lines.push('### RECOMMENDATION: REVIEW BEFORE PROCEEDING');
    lines.push('');
    lines.push('One or more signals did not meet the ≥95% success threshold, or issues were identified.');
    lines.push('Review failures above before recommending full cohort collection.');
    const failingSignals = allStats.filter(s => s.successRate < 95);
    if (failingSignals.length > 0) {
      lines.push('');
      lines.push('Signals requiring review:');
      for (const s of failingSignals) {
        lines.push(`- ${s.signalId} (${s.successRate}%)`);
      }
    }
  }
  ...
  lines.push('---');
  lines.push('');
  lines.push('## Run UUID Register');
  lines.push('');
  lines.push('Record these in RUN_REGISTER.md immediately.');
  lines.push('');
  lines.push('| Signal | Run UUID | Schema | Started At | Completed At | Note |');
  lines.push('|---|---|---|---|---|---|');
  for (const s of allStats) {
    const isV1 = ['SOT-SECURITYTXT-001', 'SOT-SECURITYHEADERS-001'].includes(s.signalId);
    const note = isV1 ? 'Stored in DB' : 'Not in DB — v0 schema; identify by timestamp window';
    lines.push(`| ${s.signalId} | \`${s.runId}\` | ${isV1 ? 'v1' : 'v0'} | ${s.startedAt} | ${s.completedAt} | ${note} |`);
  }
  lines.push('');
  lines.push('---');
  lines.push('');
  lines.push('## ADR-COL-002 Governance Compliance');
  lines.push('');
  lines.push('Authority: ADR-COL-002 — Parallel Signal Execution (Accepted 2026-06-19)');
  lines.push('');
  lines.push('| Constraint | Requirement | Status |');
  lines.push('|---|---|---|');
  lines.push(`| C-1 | Signal-runner concurrency limit defined in COP doc before run | COMPLIANT — SIGNAL_CONCURRENCY=${SIGNAL_CONCURRENCY} recorded in header |`);
  lines.push(`| C-2 | COP doc records PARALLEL model and per-signal timestamps | COMPLIANT — execution model in header; timestamps in Run UUID Register |`);
  lines.push(`| C-3 | First parallel run compared against IF-001-PILOT sequential baseline (COP-002 §5) | PENDING — ratio ${parallelismRatio} vs sequential baseline 1.000; record comparison in RUN_REGISTER.md |`);
  lines.push(`| C-4 | v0 schema run identification via COP timestamps | COMPLIANT — startedAt/completedAt recorded in Run UUID Register for all v0 signals |`);
  lines.push('');
  lines.push('---');
  lines.push('');
...
async function main() {
  ...
  if (!fs.existsSync(MANIFEST_PATH)) {
    ...
    console.error(`  Run select-cohort.js first to generate the pilot cohort.\n`);
    process.exit(1);
  }
  ...
  console.log(`\n${HR}`);
  console.log(` IF-001-PILOT — Signal Lab Collection Orchestrator`);
  ...
  console.log(`\n${HR}`);
  console.log(` IF-001-PILOT — COLLECTION COMPLETE`);
  console.log(`${HR}\n`);
  ...
  console.log(`  Run UUIDs — record these in RUN_REGISTER.md:\n`);
  for (const s of allStats) {
