...
// Retrieve ONCE; run BOTH detectors independently on the same content.
async function processDomain(domain) {
  const discovery = await discoverPolicy(domain);
  if (discovery.status !== 'located') {
    const prov = provenance(discovery, null, null);
    return { currency: currencyRow(domain, discovery.status, prov, null), retention: retentionRow(domain, discovery.status, prov, null) };
  }
...
async function main() {
  ...
  for (const t of [CURRENCY.table, RETENTION.table]) {
    const { error } = await client.from(t).select('id').limit(1);
    if (error) { console.error(`\n  ERROR: ${t} not found / unreachable — apply migrations 019 + 020. ${error.message}\n`); process.exit(1); }
  }
  let { firmCount, domains } = loadUniqueDomains();
  if (LIMIT) domains = domains.slice(0, LIMIT);

  console.log('═'.repeat(64));
  console.log(` Category H SHARED co-collection — ${COHORT_CODE} (H-SIG-001 + H-SIG-002)`);
  console.log(` run_id=${RUN_ID}  firms=${firmCount}  unique_domains=${domains.length}  concurrency=${CONCURRENCY}`);
  console.log('═'.repeat(64));

  const stats = { cur: { present: 0, absent: 0, indeterminate: 0 }, ret: { present: 0, absent: 0, indeterminate: 0 }, located: 0, written: 0, defects: [] };
  let done = 0;
  await runWithConcurrency(domains, async (domain) => {
    try {
      const { currency, retention } = await processDomain(domain);
      ...
      console.log(`  [${pct}%] ${currency.located_state.padEnd(17)} cur=${(currency.evidence_state || '-').padEnd(13)} ret=${(retention.evidence_state || '-').padEnd(13)} ${domain}`);
    } catch (err) {
  ...
  await closeBrowser();
  console.log('\n' + '─'.repeat(64));
  console.log(` SUMMARY  run_id=${RUN_ID}  (one retrieval per domain; two evidence rows)`);
  console.log(` located=${stats.located}/${domains.length}  both_rows_written=${stats.written}`);
