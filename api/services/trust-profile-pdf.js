...
// Commercial Polish Sprint: must match frontend/src/types/dto/category.dto.ts's
// CATEGORY_REFERENCE names exactly (word-for-word, including punctuation) —
// this PDF and the live app previously disagreed on category C's name and
// the PDF had no name at all for E-I, printing a bare letter code instead.
const CATEGORY_NAMES = {
  A: 'Email Trust',
  B: 'Domain Trust',
  C: 'Control Transparency Trust',
  D: 'TLS & Certificate Trust',
  E: 'Ownership Trust',
  F: 'Infrastructure Trust',
  G: 'Brand & Ecosystem Trust',
  H: 'Governance & Accountability Trust',
  I: 'Trust Stability Analytics',
};

// Plain-English display names for the underlying signal ids — mirrors
// frontend/src/features/organisation-detail/components/organisation-detail-page-client.tsx's
// SIGNAL_LABELS, so the exported report and the live app describe the same
// signal the same way. Falls back to the raw id for anything unmapped.
const SIGNAL_LABELS = {
  spf: 'SPF (email sender policy)',
  dkim: 'DKIM (email signing)',
  dmarc: 'DMARC (email authentication)',
  dnssec: 'DNSSEC (DNS security)',
  caa: 'CAA (certificate authorisation)',
  tls: 'TLS configuration',
  certificate: 'SSL/TLS certificate',
  mtasts: 'MTA-STS (mail transport security)',
  securitytxt: 'security.txt (disclosure policy)',
...
function componentRowHtml(item) {
  ...
  return `
    <div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid ${C.border};">
      <div style="font-size:11px;color:${C.text};flex:1;">${sanitize(signalLabel(item.id))}</div>
...
function buildHtml({ instance, organisationName, consultancyName }) {
  const { trustScore, componentBreakdown, organisationDomains, generatedAt } = instance;
  const primaryDomain = Array.isArray(organisationDomains) ? organisationDomains[0] : undefined;
...
<body>
  ...
  <div style="padding:24px 36px;">
    <div style="background:#fff;border:1px solid ${C.border};border-radius:10px;padding:20px 24px;margin-bottom:18px;">
      ...
      <div style="font-size:10px;color:${C.muted};margin-top:2px;">${primaryDomain ? sanitize(primaryDomain) + ' &middot; ' : ''}generated ${sanitize(formatDate(generatedAt))}</div>
    </div>
    ...
    <div style="margin-top:24px;padding-top:14px;border-top:1px solid ${C.border};">
      <p style="font-size:9px;color:#9CA3AF;line-height:1.7;">This report reflects the Trust Profile generated by Soterius at the timestamp shown above. It is a point-in-time record and is not recalculated after export. Prepared for ${sanitize(consultancyName || 'the requesting advisor')}.</p>
    </div>
  </div>
</body>
</html>`;
