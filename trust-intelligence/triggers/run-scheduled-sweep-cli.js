...
async function main({ deps = {} } = {}) {
  ...
  // Everything from here on — a bad TRUST_PROFILE_SWEEP_POLICY_THRESHOLD_MS
  // value included — is recorded as a crashed run on failure, so the metrics
  // table is a complete record of every invocation attempt, not just ones
  // that got as far as calling runScheduledSweep() itself.
  ...
  try {
    ...
    const result = await runSweep({ policyThresholdMs });
